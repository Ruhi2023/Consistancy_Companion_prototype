...
def manage_callback(upd,heading, disc, implementable,status, category, id_of_idea):
    ...
    if upd == "Yes":
        ...
        if id_of_idea is not None:
            upd_query = f"update ideas set Category = '{category}', Idea_heading = '{heading}', Idea_description = '{disc}', Implementable = {implementable}, Status = '{status}' where The_date = '{id_of_idea}'"
            try:
                My_cur.execute(upd_query)
                My_db.commit()
        ...
        print("creating")
        time.sleep(1)
        insert_query = f"insert into ideas (Idea_heading, Idea_description, Category, Implementable, Status) values ('{heading}', '{disc}', '{category}', {implementable}, '{status}')"
        try:
            My_cur.execute(insert_query)
            My_db.commit()
            st.success(f"Query executed {insert_query}")
        except Exception as e:
...
with col2:
    ...
    with st.form("convertable_form"):
        ...
        submit = st.form_submit_button("Submit", on_click=manage_callback, args=(update_existing,heading, description, implementable, status, category, id_idea))