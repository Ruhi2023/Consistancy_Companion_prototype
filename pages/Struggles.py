...
def get_answer(prompt):
    ...
    addition = f"""{discreption} As we chat assess whether there are areas for improvement. 
    If you find an improvement suggestion, respond with the suggestion enclosed by the special characters #IMPSUG:STA# and #IMPSUG:END# . 
    However, only provide a suggestion when you identify a notable opportunity for enhancement, rather than every time."""
    if len(st.session_state.chat_hist_stru) > 8:
        prev = st.session_state.chat_hist_stru[-8:]
    else:
        prev = st.session_state.chat_hist_stru
    res = model.generate_content([str({"Context For refrence like HISTORY ": addition}),str({"history HISTORY For refrence like context":prev}), prompt])
    print(res.usage_metadata)
    return res.text

@st.dialog("Add Suggestion")
def see_for_improvement(suggestion, strug):
    # cleaning the suggestion text
    suggestion = suggestion.replace("'", " ")
    suggestion = suggestion.replace('"', "")
    suggestion = "".join([s for s in suggestion.splitlines(True) if s.strip("\r\n")])
    
    strug = strug.replace("'", " ")
    strug = strug.replace('"', "")
    strug = "".join([s for s in strug.splitlines(True) if s.strip("\r\n")])
    st.write(suggestion)
    col1,col2,col3 = st.columns([1,3,1])
    if col1.button("Yes"):
        try:
            My_db, My_cur = su.connnecting()
            uery = f"INSERT INTO struggles (The_struggle, The_suggestion) VALUES ('{strug}', '{suggestion}')" # (strug, suggestion))
            My_cur.execute(uery)
            My_db.commit()
